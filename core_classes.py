...
class mytfidf(BaseEstimator, TransformerMixin):
    ...
    def __init__(self):
        ...
        self.feature_names=[]
        self.tfidf=TfidfVectorizer(analyzer='word',stop_words='english',max_df=0.8,min_df=0.01,max_features=200)


    def fit(self,x,y=None):

        self.tfidf.fit(x.iloc[:,0])
        self.feature_names=[x.columns[0]+'_'+str(temp) for temp in self.tfidf.get_feature_names()]

        return self

    def transform(self,x):

        return(pd.DataFrame(data=self.tfidf.transform(x.iloc[:,0]).toarray(),
