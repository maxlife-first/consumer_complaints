# -*- coding: utf-8 -*-
"""
Created on Mon Aug 30 12:42:33 2021
...
class vectorize(BaseEstimator,TransformerMixin):
     ...
     def __init__(self):
        ...
        self.feature_names=['Consumer complaint narrative']
     ...
     def transform(self,x):
         ...
         vector=TfidfVectorizer(analyzer='word',stop_words='english',max_df=0.8,min_df=0.01,max_features=200)
        
         
         x['Consumer complaint narrative']=dx['Consumer complaint narrative'].str.replace('XXXX','')
         X = vector.fit_transform(x['Consumer complaint narrative'])
         
         dummy=pd.DataFrame(X.toarray(),columns=list(vector.get_feature_names()))
