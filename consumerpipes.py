...
class text_frequency(BaseEstimator,TransformerMixin):
    ...
    def transform(self,x) :
        ...
        for col in x.columns:
            ...
            #x[col]=pd.DataFrame(x.toarray(),columns=self.get_feature_names())
            #x[col]=x[col].toarray()
...
class ArrayTransformer(BaseEstimator,TransformerMixin):
    ...
    def transform(self, x):
        x=pd.DataFrame(x.toarray(),columns=self.get_feature_names())
